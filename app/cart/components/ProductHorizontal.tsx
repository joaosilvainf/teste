...
import { UpdateCart } from '@/app/api/cart/route';

export default function ProductCardHorizontal(props: Cart) {
  const inputId = useId();
  const min = 1;
  const max = 10;
  ...
  const pdpLink = "/products/" + props.productCode;
  var product = AllProducts.find((p) => p.code === props.productCode);
  var finalprice = 0;
  if(props.quantity > 0 && product){
     finalprice = (product.price * props.quantity);
  }
  return (
    <div className="relative flex border-b-[1px] border-neutral-200 hover:shadow-lg min-w-[1500px] max-w-[640px] p-4">
      ...
      <div className="flex flex-col pl-4 min-w-[180px] flex-1">
        ...
        <div className="items-center sm:mt-auto sm:flex">
          <span className="font-bold sm:ml-auto sm:order-1 typography-text-sm sm:typography-text-lg">{finalprice}€</span>
